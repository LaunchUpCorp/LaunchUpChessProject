...
function ChessBoard() {
    ...
    for (let rows = 0; rows < 8; rows++) {
        ...
        for (let col = 0; col < 8; col++) {
            if ((rows + col) % 2 === 0) {
                temp.push(<div className="w-[12.5%] h-[12.5%] border-2 border-black bg-amber-600"></div>);
            } else {
                temp.push(<div className="w-[12.5%] h-[12.5%] border-2 border-black bg-green-600"></div>);
            }
        }
        arr.push(temp);
    }
